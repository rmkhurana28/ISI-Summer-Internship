...
void poly_challenge(poly *c, const uint8_t seed[CTILDEBYTES]) {
  ...
  for (i = 1; i < 4; i++) {
    ...
    EVP_DigestFinal_ex(mdctx, buf + (i * 32), &md_len);
  }
  ...
  for(i = N-TAU; i < N; ++i) {
    do {
      if(pos >= 128) {
        // Regenerate buffer if we run out - also use SHA3-256
        EVP_MD_CTX *mdctx2 = EVP_MD_CTX_new();
        const EVP_MD *md2 = EVP_sha3_256();
        EVP_DigestInit_ex(mdctx2, md2, NULL);
        EVP_DigestUpdate(mdctx2, buf, 128);
        EVP_DigestFinal_ex(mdctx2, buf, &md_len);
    ...
    // TWEAK 2: Coefficients in {-2,-1,0,1,2} - EXACTLY as in thesis
    c->coeffs[b] = (signs & 7) % 5 - 2;
    signs >>= 3;
    
    // If we've used 21 bits or more, refill values
    if(pos >= 8 && (i % 21) == 0) {
      if(pos + 8 <= 128) {
        signs = 0;
