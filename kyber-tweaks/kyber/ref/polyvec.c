...
#else
#error "KYBER_POLYVECCOMPRESSEDBYTES needs to be in {288*KYBER_K, 320*KYBER_K, 352*KYBER_K, 384*KYBER_K}"
#endif
}
...
void polyvec_decompress(polyvec *r, const uint8_t a[KYBER_POLYVECCOMPRESSEDBYTES])
{
  ...
  for(i=0;i<KYBER_K;i++) {
    for(j=0;j<KYBER_N/8;j++) {
      ...
      t[2] = (a[3] >> 0) | ((uint16_t)a[4] << 8);
      t[3] = (a[4] >> 4) | ((uint16_t)a[5] << 4);
      t[4] = (a[6] >> 0) | ((uint16_t)a[7] << 8);
      t[5] = (a[7] >> 4) | ((uint16_t)a[8] << 4);
      t[6] = (a[9] >> 0) | ((uint16_t)a[10] << 8);
      t[7] = (a[10] >> 4) | ((uint16_t)a[11] << 4);
      a += 12;

      for(k=0;k<8;k++)
        r->vec[i].coeffs[8*j+k] = ((uint32_t)(t[k] & 0xFFF)*KYBER_Q + 2048) >> 12;
    }
  }
#else
#error "KYBER_POLYVECCOMPRESSEDBYTES needs to be in {288*KYBER_K, 320*KYBER_K, 352*KYBER_K, 384*KYBER_K}"
